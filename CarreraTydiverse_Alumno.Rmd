...
```{r}
cran_filtered <- cran %>%
  filter(package == "plotly" & R.version == "3.5.1")

```
6. Elige aquellas descargas realizadas desde España (ES), con versiones de R superiores a la 3.4.0
```{r}
cran_filtered_es <- cran %>%
  filter(country == "ES" & as.numeric(sub("R-", "", R.version)) > 3.4)
```

7. Elige aquellas descargas realizadas desde España (ES), o Portugal (PT).
```{r}

cran_filtered_es_pt <- cran %>%
  filter(country %in% c("ES", "PT"))

```

8. Elige aquellas desc9argas cuyo tamaño supera los 100500 bytes desde un sistema operativo  `linux-gnu`.
```{r}
descargas_filtradas <- raw %>%
  filter(size > 100500, os == "linux-gnu")


head(descargas_filtradas)
...
descargas_con_os_version <- raw %>%
  filter(os_version != "")


head(descargas_con_os_version)
